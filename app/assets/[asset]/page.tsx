...
function Row({ label, signal, sources }: { label: string; signal: Signal<string>; sources: Source[] }) {
  ...
  return (
    <div className="grid gap-x-6 gap-y-2 py-5 md:grid-cols-[170px_190px_1fr_260px] md:items-baseline">
      ...
      <dd className="text-[13px] text-ink-50 md:text-right">
        <span className={signal.verification === "UNKNOWN" ? "text-[var(--warn)]" : "text-sage"}>
...
export default async function AssetPage({ params }: PageProps<"/assets/[asset]">) {
  ...
  return (
    <article className={`tone-${data.overall_status}`}>
      ...
      <nav aria-label="Breadcrumb" className="body-sm text-ink-50">
        <Link href="/" className="transition-colors hover:text-ink">Tracked tokens</Link>
        <span aria-hidden> / </span>
        <span className="text-ink-75">{data.asset}</span>
      </nav>

      <header className="grid gap-10 pt-8 md:grid-cols-[minmax(0,7fr)_minmax(0,5fr)] md:items-end">
        <div className="border-l border-ink-50 pl-6 md:pl-10">
          <Reveal>
            <p className="eyebrow">
              {data.name} <span className="text-ink-50">·</span> {data.asset} Stock Token
            </p>
          </Reveal>
          <Reveal delay={100}>
          ...
          <Reveal delay={220}>
            ...
            <ul className="body-sm mt-5 max-w-[60ch] space-y-2 text-ink-75">
              {data.reasons.map((r) => (
                <li key={r} className="flex gap-3">
                  <span aria-hidden className="mt-[9px] h-[5px] w-[5px] flex-none rounded-full bg-ink-50" />
                  <span>{r}</span>
                </li>
              ))}
            </ul>
        ...
        <dl className="card body-sm grid grid-cols-2 gap-x-6 gap-y-5 bg-paper/85 p-6 backdrop-blur-[6px]">
          <div>
            <dt className="text-ink-50">Chain</dt>
            <dd className="mt-0.5 text-ink">{data.chain}</dd>
          </div>
          <div>
            <dt className="text-ink-50">Issuer</dt>
            <dd className="mt-0.5 text-ink">{data.issuer}</dd>
          </div>
          <div className="col-span-2">
            <dt className="text-ink-50">Token contract</dt>
            <dd className="mt-0.5 break-all font-medium text-ink">{data.token}</dd>
          </div>
          <div>
            <dt className="text-ink-50">Last updated</dt>
            <dd className="mt-0.5 text-ink"><RelativeTime iso={data.last_updated} /></dd>
          </div>
          <div>
            <dt className="text-ink-50">Machine-readable</dt>
            <dd className="mt-0.5">
      ...
      <section className="pt-4">
        <Reveal>
          <h2 data-split className="display text-[36px] md:text-[48px]">Status by category</h2>
        </Reveal>
        ...
        <dl className="mt-6 divide-y divide-hairline border-y border-hairline">
          ...
          <div className="grid gap-x-6 gap-y-2 py-5 md:grid-cols-[170px_190px_1fr_260px] md:items-baseline">
            <dt className="text-[15px] text-ink-75">Corporate action</dt>
            <dd className="flex items-center gap-3 text-[20px] font-medium">
            ...
            <dd className="text-[15px] text-ink-75">
              {ca ? (
                <>
                  ...
                  {caSource && <span className="block text-[13px] text-ink-50">{caSource.name}</span>}
                </>
              ) : (
            ...
            <dd className="text-[13px] text-ink-50 md:text-right">
              {ca ? (
                <>
                  <span className={ca.verification === "UNKNOWN" ? "text-[var(--warn)]" : "text-sage"}>{VERIFICATION_LABEL[ca.verification]}</span>
      ...
      <section className="grid gap-10 pt-20 md:grid-cols-[minmax(0,5fr)_minmax(0,7fr)]">
        <Reveal>
          <h2 data-split className="display text-[36px] md:text-[48px]">Sources</h2>
          <p className="body-md mt-4 max-w-[40ch] text-ink-75">
        ...
        <ul className="divide-y divide-hairline border-y border-hairline">
          {data.sources.map((s) => (
            <li key={s.id} className="grid gap-1 py-4 sm:grid-cols-[150px_1fr] sm:gap-6">
              <span className="text-[13px] text-ink-50">{s.kind === "onchain" ? "Onchain" : "Authoritative"}</span>
              <div>
                {s.url ? (
                  <a href={s.url} target="_blank" rel="noreferrer" className="text-ink underline decoration-ink-40 underline-offset-[3px] transition-colors hover:text-yellow">
                    {s.name}
                  </a>
                ) : (
                  <span>{s.name}</span>
                )}
                {s.reference && <span className="block break-all text-[13px] text-ink-50">{s.reference}</span>}
              </div>
            </li>
          ))}
        </ul>
      ...
      <p className="body-sm mb-40 mt-16 max-w-[70ch] text-ink-50">
        {PROVIDER_MODE === "live"
